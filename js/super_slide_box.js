...
(function($){
	$.fn.super_slide_box = function(OPTION,KEY_CALLBACK){
		...
		// 強制複製最後一個
		function BACKDOWN(){
			var LAST_POS = SSLIDE.NUM  * (SSLIDE.WIDTH - -SSLIDE.OUTER_WIDTH); // 取得最後一個位置
			
			if(SSLIDE.KEY < 0){
				
				if(SSLIDE.VERTICAL){
					THIS.find(".slide_move").css({ "top":"-"+ LAST_POS +"px" });
				}else{
					THIS.find(".slide_move").css({ "left":"-"+ LAST_POS +"px" });
				}
				
				SSLIDE.KEY = SSLIDE.NUM - 1;
			}
		}
		...
		function SLIDE_ACT(){
			...
			if(SSLIDE.VERTICAL){
				...
				if(!SSLIDE.CYCLE_ACT){
					if(SSLIDE.KEY >= SSLIDE.NUM - (SSLIDE.SHOW_NUM - 1)){
						return false;
					}
				}
